  ...
  const testDatabaseConnection = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("products")
        .select("id, name, price, category, photo_url, mrp")
        .limit(5);
                  ...
                  {products.map((product) => (
                    ...
                    >
                      <div className="search-result-image">
                        ...
                        <div className={`w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center rounded-lg ${getPhotoUrl(product.photo_url) ? 'hidden' : ''}`}>
